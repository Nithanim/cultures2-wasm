...
fn parse_GfxBobLibs(value: &String) -> GfxBobLibs {
    let mut parts: Vec<String> = value.split_whitespace().collect();
    match parts.len() {
        1 => GfxBobLibs {
            bmd: parts.remove(0).unwrap(),
            shadow: None,
        },
        2 => GfxBobLibs {
            bmd: parts.remove(0).unwrap(),
            shadow: Some(parts.remove(0).unwrap()),
        },
...
fn parse_GfxTransition<'a>(builder: &'a mut GfxLandscapeBuilder, value: &String) -> &'a GfxLandscapeBuilder {
    ...
    let mut split: Vec<String> = value.split_whitespace().collect();
    let k: u8 = split.get(0).unwrap().parse().unwrap();
    let v: String = split.remove(1).unwrap();
...
fn split_string_to_ints<T>(s: &String) -> Vec<T> {
